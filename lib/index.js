 ...
 */
Model.prototype.applyIndexes = function (options, callback) {
  ...
  if (consLen || indLen) {
    ...
    if (options && options.transaction) {
      self.db.executeStatement(options.transaction, {statements: transStatement}, function (err) {
        applyIndexes(self, options, err, callback);
      });
    }
...
function applyIndexes (self, options, err, callback) {
  if (err) {
    ...
    if (err.message && err.message.indexOf('Already constrained') !== -1) {
      ...
      self.applyIndexes(options, callback);
    }
...
// Model.prototype.parseResultsStream = function (results, options) {
//   var stream  = JSONStream.parse(['results[0].data]']);
