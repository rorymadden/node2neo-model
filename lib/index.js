...
Model.prototype.parseResults = function(results, options){
  ...
  console.log(results.results[0].columns)

  //how many columns are returned?
  while(results.results[0].columns.length > 0){
    result.push(parseRows());
    first = false;
  }

  function parseRows(){
    var id = results.results[0].data[0].row[0];
    var node = results.results[0].data[0].row[1];
    var rel, subSchemaTag;
    // remove fields not required - only on top level node
    // TODO: investigate how to remove subSchemas
    if(options && options.fields && first){
      var elements = options.fields.split(' ');
      for(var key in node){
        if(elements.indexOf(key) === -1) delete node[key];
      }
    }
    //if it is the first time check for a relationship
    if(first){
      // if there is a relationship as well
      if(typeof results.results[0].data[0].row[2] === 'string'){
        rel = results.results[0].data[0].row[3];
        rel.type = results.results[0].data[0].row[2];
        //remove these values from the result
        results.results[0].data[0].row.splice(2,4);
        results.results[0].columns.splice(2,4);
      }
    }
    // otherwise
    else {
      subSchemaTag = results.results[0].columns[1];
    }
    //remove these values from the result
    results.results[0].data[0].row.splice(0,2);
    results.results[0].columns.splice(0,2);
    ...
    if(rel) response.rel = rel;
    if(subSchemaTag) response.subSchemaTag = subSchemaTag;
  ...
  result.forEach(function(element, index){
    if(index === 0) interimResult = element;
    else {
      //remvoe the count attribute
      element.subSchemaTag = element.subSchemaTag.slice(0, element.subSchemaTag.lastIndexOf('_'));
      //remvoe the count level attribute
      element.subSchemaTag = element.subSchemaTag.slice(0, element.subSchemaTag.lastIndexOf('_'));

      //does interimResult already have a value for element
      if(interimResult[element.subSchemaTag]){
        if(Array.isArray(interimResult[element.subSchemaTag])){
          interimResult[element.subSchemaTag].push(element);
        }
        else {
          interimResult[element.subSchemaTag] = [interimResult[element.subSchemaTag]];
          interimResult[element.subSchemaTag].push(element);
        }
      }
      //otherwise just add it
      else interimResult[element.subSchemaTag] = element;
      delete interimResult[element.subSchemaTag].subSchemaTag;
    }
  });
