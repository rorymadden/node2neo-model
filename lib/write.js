...
module.exports.create = function (data, options, callback) {
    ...
    create: function (cb) {
      ...
      if (options.relationship) {
        ...
        options.relationship.forEach(function (element, index) {
          ...
          if (element.nodeLabel) {
            matchQuery += '(relNode' + index + ':' + element.nodeLabel + ')';
          }
          else {
            matchQuery += '(relNode' + index + ')';
          }
      ...
      if (options.transaction) {
        ...
        self.db.executeStatement(options.transaction, {statements: [statement] }, function (err, response) {
          if (err) {
            return cb(err);
          }
          else {
  ...
  function (err, result) {
    if (err && options.transaction) {
      self.db.removeTransaction(options.transaction, function () {
        return callback(err);
      });
    }
